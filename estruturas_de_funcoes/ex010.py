# 10.Jogo de Craps. Faça um programa de implemente um jogo de Craps. O jogador lança um par de dados, obtendo um valor entre 2 e 12. Se, na primeira jogada, você tirar 7 ou 11, você um "natural" e ganhou. Se você tirar 2, 3 ou 12 na primeira jogada, isto é chamado de "craps" e você perdeu. Se, na primeira jogada, você fez um 4, 5, 6, 8, 9 ou 10,este é seu "Ponto". Seu objetivo agora é continuar jogando os dados até tirar este número novamente. Você perde, no entanto, se tirar um 7 antes de tirar este Ponto novamente.
from random import randint

print('-=' *10)
print('     Bem-Vindo')
print('-=' *10)
def craps():
    dado1 = randint(1, 6)
    dado2 = randint(1, 6)
    dados = dado1 + dado2
    while True:
        dados = randint(2, 13)
        
        if dados == 7 or dados == 11:
            print(f'Você tirou o número {dados} e ganhou! É natural.')
            break
        elif dados == 2 or dados == 3 or dados == 12:
            print(f'Você tirou o número {dados} e perdeu! É um craps.')
            break
        else:
            print('Seu ponto é', dados)
            ponto = dados
            continuar = True
            
            while continuar:
                dado1 = randint(1, 6)
                dado2 = randint(1, 6)
                dados = dado1 + dado2
                print('Você lançou novamente e obteve o número', dados)

                if dados == ponto:
                    print('Você ganhou! Tirou seu ponto novamente.')
                    continuar = False
                elif dados == 7:
                    print('Você perdeu! Tirou um 7 antes de tirar seu ponto.')
                    return False
craps()